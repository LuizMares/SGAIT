...
export const dbService = {
  ...
  async getCurrentUser(passedUser?: any): Promise<UserProfile | null> {
    try {
      ...
      let userRole = UserRole.AGENTE;
      if (cleanEmail === 'sttpojucaba@gmail.com' || cleanEmail === 'luizemerson17@gmail.com') {
        userRole = UserRole.ADMIN;
      } else {
        try {
          userRole = await determineUserRole(cleanEmail);
        } catch (e) {
          userRole = UserRole.AGENTE;
        }
      }

      // Extract user display name and avatar from OAuth metadata or email
      const meta = user.user_metadata || {};
      const userName = meta.full_name || meta.name || meta.preferred_username || cleanEmail.split('@')[0];
      const userAvatar = meta.avatar_url || meta.picture || `https://api.dicebear.com/7.x/adventurer/svg?seed=${encodeURIComponent(userName || cleanEmail)}`;

      // Build the user profile
      const profileData: UserProfile = {
        id: user.id || `user-${cleanEmail.replace(/[^a-zA-Z0-9]/g, '')}`,
        email: cleanEmail,
        name: userName,
        avatarUrl: userAvatar,
        role: userRole,
        googleId: user.id || `google-${cleanEmail}`,
        firstAccessAt: user.created_at || new Date().toISOString(),
        lastLoginAt: new Date().toISOString()
      };

      // Save to local storage cache immediately so session is remembered
      safeStorage.setItem(STORAGE_KEYS.CURRENT_USER, JSON.stringify(profileData));

      // Try to sync with Supabase tables if active, non-blocking
      if (isSupabaseConfigured() && supabaseClient) {
        try {
          ...
          console.warn('Non-fatal error upserting sgait_profiles:', e);
        }
      }
  ...
  async getTickets(options?: { page?: number; limit?: number; last24h?: boolean; search?: string }): Promise<TrafficTicket[]> {
    ...
    // 1. Fetch from Supabase using standard .select() query
    if (isSupabaseActive()) {
      try {
    ...
    // 2. Fetch from central backend server API if Supabase returned empty
  ...
  async insertTicket(ticket: Omit<TrafficTicket, 'id' | 'createdAt' | 'updatedAt'>): Promise<{ data: TrafficTicket | null; error: string | null }> {
    ...
    // 3. Direct .insert() into Supabase table if active
    if (isSupabaseActive()) {
      try {
        ...
        if (error && (error.code === '23505' || error.code === '42P10' || (error.message && error.message.toLowerCase().includes('conflict')))) {
        ...
        if (error) {
          console.warn('Supabase insert warning in insertTicket:', error);
        }
      } catch (err: any) {
  ...
  // ==========================================
  // MANUAL FETCH & DATA REFRESH (Realtime/Subscriptions Disabled)
  // ==========================================
  
  subscribeTickets(_callback?: (message: RealtimeMessage) => void): () => void {
    // Realtime subscriptions and background channels are disabled.
    // The application uses manual fetches (.select()) via the "Atualizar" button.
    return () => {};
  }
};
