...
export default function LoginView({ onLoginSuccess }: LoginViewProps) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (href.includes('error=') || hash.includes('error=')) {
        try {
          const urlObj = new URL(window.location.href.replace('#', '?'));
          const errDesc = urlObj.searchParams.get('error_description') || urlObj.searchParams.get('error') || 'Falha no login com Google.';
          setErrorMessage(decodeURIComponent(errDesc));
        } catch (e) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setErrorMessage(err.message || 'Erro ao realizar login via Google.');
      setIsLoading(false);
    }
  };
        ...
        <div className="p-6 space-y-5 flex-1 text-slate-700 flex flex-col justify-between bg-slate-50/50">
          ...
          <div className="text-center space-y-1 pt-2">
            ...
            <p className="text-xxs text-slate-500 max-w-xs mx-auto leading-relaxed">
              Autentique-se utilizando sua conta Google institucional para acessar o painel de trânsito.
            </p>
          </div>
          ...
          <div className="py-2">
            <button
              ...
              disabled={isLoading}
              className="w-full py-4 px-4 bg-white hover:bg-slate-100 border-2 border-slate-300 text-slate-800 font-extrabold text-xs tracking-wide rounded-2xl shadow-md flex items-center justify-center gap-3 transition-all cursor-pointer hover:shadow-lg active:scale-[0.99]"
            >
              {isLoading ? (
          ...
          <div className="flex items-center justify-center gap-1.5 text-xxs font-mono text-slate-400 pt-3 border-t border-slate-200">
            ...
            <span>
              Autenticação Social: <b>Google OAuth via Supabase</b>
            </span>
          </div>
